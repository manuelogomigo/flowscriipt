document.addEventListener("DOMContentLoaded", function () {
  ...
  countdownWrappers.forEach(function (countdownWrapper) {
    ...
    const countdownMinutes = countdownWrapper.querySelector(
      "[ct-countdown-minutes]",
    );
    const countdownSeconds = countdownWrapper.querySelector(
      "[ct-countdown-seconds]",
    );
    const countdownDate = countdownWrapper.getAttribute("ct-countdown-date");
    const countdownTime =
      countdownWrapper.getAttribute("ct-countdown-time") || "12:00am";
    ...
    const formattedCountdownTime = `${hours24
      .toString()
    ...
    const countdownTimestamp = new Date(
      `${formattedCountdownDate} ${formattedCountdownTime}`,
    ).getTime();
